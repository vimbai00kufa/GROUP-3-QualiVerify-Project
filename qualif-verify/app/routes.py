...
def current_actor(x_user: str = Header(default="anonymous")) -> str:
    """The identity of the caller (simplified for the demo — see report)."""
    return x_user or "anonymous"


def require_admin(x_role: str = Header(default="")) -> str:
    if x_role != "admin":
        raise HTTPException(status_code=403, detail="Admin role required")
    return x_role


def require_verifier(x_role: str = Header(default="")) -> str:
    if x_role not in ("admin", "verifier"):
        raise HTTPException(status_code=403, detail="Verifier or admin role required")
    return x_role
...
def search_qualifications(
    ...
    db: Session = Depends(get_db),
    role: str = Depends(require_verifier),
    actor: str = Depends(current_actor),
):
