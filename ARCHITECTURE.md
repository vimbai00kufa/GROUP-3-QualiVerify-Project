...
5. **Header-based roles (`X-Role` / `X-User`) — a deliberate limitation.**
   ...
   upgrade path. (Swapping in JWT is itself a bonus opportunity.) The full
   four-role RBAC model built on top of this header scheme — roles,
   permission matrix, and enforcement — is documented separately in
   `docs/RBAC.md`.
