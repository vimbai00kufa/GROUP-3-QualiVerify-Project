...
from conftest import ADMIN_HEADERS, NO_ROLE_HEADERS, REGISTRAR_HEADERS, VERIFIER_HEADERS
...
def test_register_requires_admin_or_registrar_role(client):
    """Recognised role, insufficient permission -> 403 (authorisation)."""
    assert _register(client, headers=VERIFIER_HEADERS).status_code == 403


def test_register_registrar_is_permitted(client):
    """Registrar was extended the same registration right as Administrator."""
    assert _register(client, headers=REGISTRAR_HEADERS).status_code == 201


def test_register_requires_authentication(client):
    """No X-Role header at all -> 401 (authentication), not 403."""
    assert _register(client, headers=NO_ROLE_HEADERS).status_code == 401
...
def test_search_requires_role(client):
    """No X-Role header -> 401 (authentication required, not merely forbidden)."""
...
def test_audit_allows_verification_officer(client):
    """RBAC matrix extends audit read access to Verification Officer too."""
    assert client.get(f"{BASE}/audit", headers=VERIFIER_HEADERS).status_code == 200


def test_audit_requires_admin_or_verification_officer(client):
